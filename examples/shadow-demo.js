...
export class Shadow_Demo extends Scene {
    ...
    display(context, program_state) {
        ...
        const view_mat = Mat4.look_at(vec3(0, 10, 5), vec3(0, 0, -5), vec3(0, 0, -1));
        const proj_mat = Mat4.perspective(Math.PI/3, 1, 0.8, 500);
        // Bind the Depth Texture Buffer
        gl.bindFramebuffer(gl.FRAMEBUFFER, this.lightDepthFramebuffer);
        gl.viewport(0, 0, this.lightDepthTextureSize, this.lightDepthTextureSize);
        gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);
        // Prepare uniforms
        program_state.light_view_mat = view_mat;
        program_state.light_proj_mat = proj_mat;
        program_state.light_tex_mat = proj_mat;
        program_state.view_mat = view_mat;
        program_state.projection_transform = proj_mat;
